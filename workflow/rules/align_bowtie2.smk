...
if not is_single_end():
    rule bowtie2_pe:
        ...
        output:
            bam = maybe_temp(f"{OUTDIR}/bowtie2/{{sample}}/{{sample}}_pe.bam"),
            bai = maybe_temp(f"{OUTDIR}/bowtie2/{{sample}}/{{sample}}_pe.bam.bai"),
    ...
    rule bowtie2_se:
        ...
        params:
            ...
            mode = config.get("bowtie2", {}).get("mode", "very-sensitive")
        output:
            bam = maybe_temp(f"{OUTDIR}/bowtie2/{{sample}}/{{sample}}_se.bam"),
            bai = maybe_temp(f"{OUTDIR}/bowtie2/{{sample}}/{{sample}}_se.bam.bai"),
            flagstat = aligned_flagstat_path("{sample}")
        log:
...
rule filter_unique_mappers:
    ...
    output:
        bam = maybe_temp(f"{OUTDIR}/bowtie2/{{sample}}/{{sample}}.unique.bam"),
        bai = maybe_temp(f"{OUTDIR}/bowtie2/{{sample}}/{{sample}}.unique.bam.bai"),
